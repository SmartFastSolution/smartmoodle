...
class TallerEstudianteController extends Controller
{
    ...
    public function taller($plant, $id)
    {
        ...
        if ($resp >= 1) {
            ...
            if ($plant == 1) {
                ...
                $consul = Taller::findorfail($id);
                $datos = Taller::where('taller_id', $consul->id)->firstOrFail();
    ...
    public function store5(Request $request, $idtaller)
    {
        ...
        foreach ($enun as $key => $value) {
            $datos = array(
                ...
                'respuesta' => isset($request->respuesta[$key]) ? $request->respuesta[$key] : null,
                'created_at' => now(),
                'updated_at' => now(),
            );
    ...
    public function store10(Request $request, $idtaller)
    {
        ...
        foreach ($request->order as $key2 => $cd) {
            if ($cd > $conteo or null) {
                ...
                $datos[] = isset($alter2[$cd - 1]->definicion_aleatoria) ? $alter2[$cd - 1]->definicion_aleatoria : null;
            }
        }
        // return $datos;
    ...
    public function complete31(Request $request, $idtaller)
    {
        $id                                          = Auth::id();
        $user                                        = User::find($id);
        $user->tallers()->attach($idtaller, ['status' => 'completado', 'fecha_entregado' => now()]);
    ...
    public function complete48(Request $request, $idtaller)
    {
        $id                                          = Auth::id();
        $user                                        = User::find($id);
        $user->tallers()->attach($idtaller, ['status' => 'completado', 'fecha_entregado' => now()]);
