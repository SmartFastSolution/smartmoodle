<div id="conciliacionb" class="border border-danger p-4">
    ...
    <div class="row justify-content-around mb-2">
        ...
        <a data-toggle="modal" data-target="#c_saldos" @click="limpiar()" class="btn btn-outline-primary">Agregar
            Saldo</a>
        <a data-toggle="modal" data-target="#c_debitos" @click="limpiar()" class="btn btn-outline-success">Agregar
            Débitos</a>
        <a data-toggle="modal" data-target="#c_creditos" @click="limpiar()" class="btn btn-outline-warning">Agregar
            Crédito</a>
        <a data-toggle="modal" data-target="#c_cheques" @click="limpiar()" class="btn btn-outline-danger">Agregar
            Cheque</a>
    </div>
            ...
            <draggable class="list-group list-group-flush" :list="c_saldos" group="people">
                <div v-for="(element, index) in c_saldos" :key="element.name">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @{{ element.detalle }}<span class="badge-pill">@{{ decimales(element.saldo) }} <a @click="editSaldo(index)"
        ...
        <div class="col-7">
            ...
            <draggable class="list-group list-group-flush" :list="c_debitos" group="people">
                <div v-for="(element, index) in c_debitos" :key="element.name">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @{{ element.detalle }}<span class="badge-pill">@{{ decimales(element.saldo) }} <a @click="editDebitos(index)"
                                class="btn btn-warning btn-sm mr-1"><i class="fas fa-edit"></i></a><a
    ...
    <div class="row">
        <div class="col-7">
            <h3 class="text-left font-weight-bold text-danger"> - CRÉDITOS BANCARIOS NO CONTABILIZADOS</h3>
            <draggable class="list-group list-group-flush" :list="c_creditos" group="people">
                <div v-for="(element, index) in c_creditos" :key="element.name">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @{{ element.detalle }}<span class="badge-pill">@{{ decimales(element.saldo) }} <a @click="editCreditos(index)"
                                class="btn btn-warning btn-sm mr-1"><i class="fas fa-edit"></i></a><a
        ...
        <div class="col-7">
            ...
            <draggable class="list-group list-group-flush" :list="c_cheques" group="people">
                <div v-for="(element, index) in c_cheques" :key="element.name">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        @{{ element.detalle }}<span class="badge-pill">@{{ decimales(element.saldo) }} <a @click="editCheques(index)"
                                class="btn btn-warning btn-sm mr-1"><i class="fas fa-edit"></i></a><a
    ...
    <div class="row justify-content-center">
        <a class="btn p-2 mt-3 btn-outline-info" @click.prevent="guardarConciliacionB()">Guardar Conciliación Bancaria</a>
    </div>
