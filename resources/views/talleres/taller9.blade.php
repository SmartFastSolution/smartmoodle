    ...
    <form action="{{ route('taller9', ['idtaller' => $d]) }}" method="POST" id="taller9">
        ...
        <div class="container">
            @foreach ($datos->TallerSubraOps as $key => $element)
                <div class="row mb-5 justify-content-center bg-secondary p-2"
                    style="box-shadow: 5px 5px 15px 0px  #74ECF7;">
                    <div class="col align-content-center">
                        ...
                        <div class="row justify-content-start">
                            @foreach ($info = explode(',', $element->alternativas) as $e)
                                <div class="col-2 align-content-center"><input type="radio"
                                        name="respuesta[{{ $key }}]" value="{{ trim($e) }}">
                                    <label>{{ trim($e) }}</label></div>
                            @endforeach
                        </div>
                    </div>
                </div>
