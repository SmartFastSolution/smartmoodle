    ...
    var WordFindGame = function () {
        ...
        */
        var drawWords = function (el, words) {
            ...
            var output = '<ul class="nav flex-column">';
            // output +=  '<h4 class="text-info text-center">Palabras relacionadas al comercio </h4>'
            for (var i = 0, len = words.length; i < len; i++) {
                var word = words[i];

                output += '<li class="nav-item  ' + word + '">' + [i + 1] + '. ' + word;
            }
            output += '</ul>';
        ...
        var endTurn = function () {
            ...
            for (var i = 0, len = wordList.length; i < len; i++) {
                ...
                if (wordList[i] === curWord) {
                    ...
                    wordList.splice(i, 1);
                    EventBus.$emit('palabra', curWord);
                    $('.' + curWord).addClass('wordFound');
                }

                if (wordList.length === 0) {
                    //$("#activar").attr("disabled", "");}
